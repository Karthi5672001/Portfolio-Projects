...
Create View Sumofpeopledeadpercent as
Select dea.continent, dea.location, dea.date, population, dea.new_deaths
, SUM(CONVERT(int, dea.new_deaths)) OVER (Partition by dea.location Order by dea.location, dea.Date ) as Sumofpeopledead
From [Portfolio Project1]..Covidvaccs$ vac
join [Portfolio Project1]..coviddeaths$ dea
on dea.location = vac.location
and dea.date = vac.date
...
Create View Sumofpeoplevaccinatedpercentuk as
Select dea.continent, dea.location, dea.date, population, vac.new_vaccinations
, SUM(CONVERT(int, vac.new_vaccinations)) OVER (Partition by dea.location Order by dea.location, dea.Date ) as Sumofpeoplevaccinated
From [Portfolio Project1]..Covidvaccs$ vac
join [Portfolio Project1]..coviddeaths$ dea
...
Create View Sumofpeopledeadpercentuk as
Select dea.continent, dea.location, dea.date, population, dea.new_deaths
, SUM(CONVERT(int, dea.new_deaths)) OVER (Partition by dea.location Order by dea.location, dea.Date ) as Sumofpeopledead
From [Portfolio Project1]..Covidvaccs$ vac
join [Portfolio Project1]..coviddeaths$ dea
